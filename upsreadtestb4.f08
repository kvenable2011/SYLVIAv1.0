program newinput
IMPLICIT NONE
real :: state_att(6),int_con(4),cumm_freq(7),state_var(6)!fc(7)
...
real(kind=4), allocatable::mad(:), mad_b(:), mad_1(:), ml_ly1(:), ml_ly2(:), ml_ly3(:)!,freq_count(:)
real(kind=4), allocatable::tp(:), tp_1(:), tp_b(:), tp_s(:)!placeholders for move_alloc
...
integer :: irow, rtm, tinc, so,yn, i, j, J1, new_end_step, next_row, comid, tp_1_size, tp_size, tp_b_size
...
INTEGER(KIND=4) :: nr, k, start_j, jj
...
common /shear/tb 
open(unit=10, file="C:\Users\KVENABLE\SYLVIA\Output\GlobalStateFile12Jun25d.txt",status="new")
call cpu_time(start)
!call date_and_time
open(unit=15, file="C:\Users\KVENABLE\SYLVIA\Output\XY12Jun25d.txt",status="new")
open(unit=25, file="C:\Users\KVENABLE\SYLVIA\Output\mass12Jun25d.txt", status="new")
open(unit=35, file="C:\Users\KVENABLE\SYLVIA\Output\conc12Jun25d.txt", status="new")
open(unit=45, file="C:\Users\KVENABLE\SYLVIA\Output\mad12Jun25d.txt", status="new")
open(unit=55, file="C:\Users\KVENABLE\SYLVIA\Output\ml12Jun25d.txt", status="new")
open(unit=65, file="C:\Users\KVENABLE\SYLVIA\Output\inputmatrixpass12Jun25d.txt", status="new")
...
DO  k=1,nr
 ...
 dx = mydata(6,k)! * 3600!displacement in 1 hr (m/hr) --> activate with 56
 ...
 if (k > 1 .and. j1 >= rtm) then 
    ...
    lsv = (mydata(6,nint(stp))) !Activate with line 56
...
!        cumm_freq = (fc/ns)*100 !As percentage
    ...
    !lnumb= i*1 - place statement here for else when i==ns-- down below
        if (i<ns) then 
        maxlyrd=dlayer* real(i)
        call passed2(state_var,i,m_1,maxlyrd,tsf,pdx,pdy,state_att,a,tb,dprob) !freq_count)!taub
           if (i==1) then
            allocate(state_var_time_1, SOURCE=state_var) 
            !state_var_time_1=state_var(1:4)
            ...
            call passed3 (state_var,i,m_1,maxlyrd_b,tsf,pdx,pdy,state_att,a,tb,dprob)!freq_count !taub
             !maxlyrd_b=h+ 0.1
    !This is where the new type of memory allocation should begin using MALLOC AND MOVE_ALLOC
            allocate(state_var_time_3, SOURCE=state_var) 
            !state_var_time_3=state_var(1:6)
...
!            exit !Use these parameters to calc next pdx final displacement in network
        ...
        pdx=(dx*real(j-1))/1000 !Since this is in scope at all times in, not needed to include in allocated space. Just need to print out 
        tsv_1 =(state_var_time_1(3)/(1000*state_att_1(4,1)))/1000!Total Settling Velocity LY1
        tsv_2 =(state_var_time_2(3)/(1000*state_att_1(4,2)))/1000!Total Settling Velocity LY2
        tsv_3 =(state_var_time_3(3)/(1000*state_att_1(4,3)))/1000!Total Settling Velocity LY3
...
!        mad = [mad,real(rtm)]
!        mad_1 = [mad_1,real(rtm)]
!        mad_b = [mad_b,real(rtm)]
!        tot_loss_ly1 = [tot_loss_ly1,real(rtm)]
!        tot_loss_ly2 = [tot_loss_ly2,real(rtm)]
!        tot_loss_ly3 = [tot_loss_ly3,real(rtm)]
!        ml_ly1 = [ml_ly1, real(rtm)]
!        ml_ly2 = [ml_ly2, real(rtm)]
!        ml_ly3 = [ml_ly3, real(rtm)]
        !if (j==1) then
        ...
        timeout(2,1:2)=state_var_time_1(2)-(d_m_1*pdx)!change in mass flux
        timeout(3,1)=timeout(2,j)/svol!concentration scaling factor
        !tp(j) = timeout(2,j:j-1)) !only mass is necessary 
        !tp(j) = (timeout(2,j)) !only mass is necessary 
        !tp_size = size(tp)
        !tp_s = [int(timeout(2,1:j-1)), real(tp(j)), int(timeout(2,j:))]
!        if (j<=2) then
        !tp_s = [int(tp(1:j-1)), nint(timeout(2,j)), int(tp(j:))]
        tp_s = [(timeout(2,1:int(j-1))), (timeout(2,int(j))), timeout(2,j:)]
        !tp_s = [int(timeout(2,1:j-1)), nint(timeout(2,j)), int(timeout(2,j:))]
        !tp_s = [int(tp(1:j-1)), j, int(tp(j:))]
        print*, size(tp_s)! for debugging
        !call move_alloc(tp_s, mad)
            !allocate(mad(2)) !, SOURCE=timeout(2,j:rtm))
            !mad(2) = timeout(2,j)
!        end if 
        !tp_s = [int(mad(1:j-1)),nint(tp(j)), int(mad(j:))]
        if (j> 1) then
            !tp_s(j) = (timeout(2,j)) 
            tp_s = [(mad(1:int(j-1))), timeout(2,int(j)), mad(j:)]
            !tp_s = [int(mad(1:j-1)), nint(timeout(2,j)), int(mad(j:))]
            !tp_s = [int(mad(1:j-1)), j, int(mad(j:))]
            !print*, size(tp_s)! for debugging
            timeout(2,3:rtm)=mad(j-1)-(d_m_2*pdx)!change in mass flux
            !call move_alloc(tp_s, mad)
            !call move_alloc(mad, tp_s) !maybe flip flop
        end if    
!        if (j==1) then
        call move_alloc(tp_s, mad)
!        end if     
        print*, size(mad)
        !end if     
!        timeout_1(2,3:rtm)=mad(2,j-1)-(d_m_2*pdx)!change in mass flux
!            allocate(mad(j:rtm), SOURCE=timeout(2,j:rtm))!need to establish where j/k= 1
!            mad(j) = timeout(2,j)
!            timeout(2,3:rtm)= mad(j-1)-(d_m_1*pdx)!change in mass flux
        !end if--> hope correct truncate     
        !mad(j) = timeout(2,j)
        timeout(3,2:rtm)=timeout(2,j)/svol!concentration scaling factor in mg/L
        !allocate(timeout_1(3,rtm))
        timeout_1(1,j)=(state_var_time_2(4)-dlayer)+(tsv_2*real(j-1))!y displacement               !y displacement
        timeout_1(2,1:2)=state_var_time_2(2)-(d_m_2*pdx)!change in mass flux
        timeout_1(3,1)=timeout_1(2,j)/svol!concentration scaling factor
        tp_1 = [timeout_1(2,1:int(j-1)), timeout_1(2,int(j)), timeout_1(2,j:)]
        !tp_s = [int(tp(1:j-1)), j, int(tp(j:))]
        print*, size(tp_1)! for debugging
        if (j> 1) then
            tp_1 = [mad_1(1:int(j-1)), timeout_1(2,int(j)), mad_1(j:)]    
            timeout_1(2,3:rtm)=mad_1(j-1)-(d_m_2*pdx)!change in mass flux
        end if
        call move_alloc(tp_1, mad_1)
        print*, size(mad_1)
!        if (j ==2) then
!            allocate(mad_1(2:rtm)) !, SOURCE=timeout(2,j:rtm))
!            mad_1(2) = timeout_1(2,j)
!        end if    
!        if (j> 2) then
!            !call move_alloc(timeout_1, mad_1)
!            !allocate(mad_1(j:rtm)) 
!            do   jj= 2,j
!                mad_1(j) = timeout_1(2,jj)
!            end do 
!            timeout_1(2,3:j)=mad_1(j-1)-(d_m_2*pdx)!change in mass flux
!        end if     
!        timeout_1(2,3:rtm)=mad_1(2,j-1)-(d_m_2*pdx)!change in mass flux
        !end if --> hope right end comment
        !mad_1(j) = timeout_1(2,j)
        timeout_1(3,2:rtm)=timeout_1(2,j)/svol!concentration scaling factor
        !allocate(benthic_layer_3(5,rtm))
        ...
        benthic_layer_3(2,1:2)= state_var_time_3(2)-(d_m_3*pdx)!change in mass flux
        ...
        tp_b = [(benthic_layer_3(2,1:int(j-1))), benthic_layer_3(2,int(j)), benthic_layer_3(2,j:)]
        print*, size(tp_b)! for debugging
        if (j> 1) then
            tp_b = [(mad_b(1:int(j-1))), benthic_layer_3(2,int(j)), mad_b(j:)]    !needs to be benthic 3 6/13
            benthic_layer_3(2,3:rtm)=mad_b(j-1)-(d_m_2*pdx)!change in mass flux(2,3:j)=mad_1(j-1)-(d_m_2*pdx)!change in mass flux
        end if
        call move_alloc(tp_b, mad_b)
        print*, size(mad_b)
        !benthic_layer_3(3,2:rtm)=mad_b(j-1)-(d_m_2*pdx)!change in mass flux
!        if (j ==2) then
!            allocate(mad_b(2:rtm)) !, SOURCE=timeout(2,j:rtm))
!            mad_b(2) = benthic_layer_3(2,j)
!        end if
!        if (j> 2) then
!            !call move_alloc(benthic_layer_3, mad_b)
!            !allocate(mad_b(j:rtm)) 
!            do   jj= 2,j
!                mad_b(jj) = benthic_layer_3(2,jj)
!            end do 
!            benthic_layer_3(2,3:j)=mad_b(j-1)-(d_m_2*pdx)!change in mass flux
!        end if     
!        benthic_layer_3(2,3:rtm)=mad_b(2,j-1)-(d_m_2*pdx)!change in mass flux
!            allocate(mad_b(j:rtm), SOURCE=benthic_layer_3(2,j:rtm))
!            mad_b(j) = benthic_layer_3(2,j)
!            benthic_layer_3(2,3:rtm)= mad_b(j-1)- (d_m_3*pdx) !change in mass flux
        !end if --> hope right truncation
        !mad_b(j) = benthic_layer_3(2,j)
...
!        mad_b(j) = benthic_layer_3(2,j)
        ...
        ml_ly1(1) = 0
        if (j>= 2) then 
!            ml_ly1(2) = tp_s(j) - state_var_time_1(2)
!            ml_ly1(3:j) = tp_s(j) - tp_s(j-1)
            ml_ly1(2) = mad(j) - state_var_time_1(2)
            ml_ly1(3:j) = mad(j) - mad(j-1)
        end if
        if(j==1) then 
            allocate(ml_ly2(rtm))
        end if 
        ml_ly2(1) = 0
        if (j>= 2) then 
            ml_ly2(2) = mad_1(j) - state_var_time_2(2)
            ml_ly2(3:j) = mad_1(j) - mad_1(j-1)
        end if    
        if(j==1) then 
            allocate(ml_ly3(rtm))
        end if     
        ml_ly3(1) = 0
        if (j>= 2) then 
            ml_ly3(2) = mad_b(j) - state_var_time_3(2)
            ml_ly3(3:rtm) = mad_b(j) - mad_b(j-1)
        end if 
        if(j==1) then 
            allocate(tot_loss_ly1(rtm))
        end if    
        tot_loss_ly1(j) = sum(ml_ly1)
        if(j==1) then 
            allocate(tot_loss_ly2(rtm))
        end if
        tot_loss_ly2(j) = sum(ml_ly2)
        if(j==1) then 
            allocate(tot_loss_ly3(rtm))
        end if    
        tot_loss_ly3(j) = sum(ml_ly3)
        if(timeout(2,j)<0) then 
            timeout(2,j) =0
        end if     
        if(timeout_1(2,j)<0) then 
            timeout_1(2,j) =0
        end if
...
!        deallocate(tot_loss_ly3)
        ...
        !end if 
        if (j>sumcount) then 
        new_end_step =sumcount+nint(next_time)
