...
real, dimension (4,1):: state_att_1,state_att_2, state_att_3
real, dimension (4):: state_att 
real, dimension (5):: state_var_time_1, state_var_time_2, state_var_time_3, state_var
real, dimension (:,:), allocatable:: benthic_layer_3, bottom_out
...
real :: pdx, pdy, maxlyrd, dx, st, dy, dm, ddtb, dtb, m, vol, wb, bf, dh, xks
...
!lnumb= i*1 - place statement here for else when i==ns-- down below
    if (i<ns) then 
    maxlyrd=dlayer* real(i)   
    call passed1(state_var,i,rhos,sv, maxlyrd,state_att,a,freq_count) 
        if (i==1) then
        state_var_time_1=state_var(:)
...
!num_par_1 = conc_i*lc*a
    ...
    timeout(5,j) = state_var_time_1(5)
    timeout(6,j) = pdx
    timeout(7,j) = (timeout(4,j)-dlayer)+(timeout(3,j)*real(j))
    timeout(8,j) = timeout(2,j)-((timeout(2,j)/dx_1)*pdx)
    timeout(9,j) = (state_var_time_1(2)/(svol*10**6)) - (pdx*(state_var_time_1(2)/(svol*10**6))/dx_1) 
    ...
    timeout_1(5,j) = state_var_time_2(5)
    timeout_1(6,j) = pdx
    timeout_1(7,j) = (timeout_1(4,j)-dlayer)+(timeout(3,j)*real(j))
    timeout_1(8,j) = timeout_1(2,j)-((timeout_1(2,j)/dx_1)*pdx)
    timeout_1(9,j) = (timeout_1(2,j)/(svol*10**6)) - (pdx*(timeout_1(2,j)/(svol*10**6))/dx_1) 
    allocate(benthic_layer_3(9,rtm))
    !benthic_layer_3(0,j)= 1
    ...
    benthic_layer_3(6,j)= pdx
    benthic_layer_3(7,j)= ((maxlyrd_b-0.1)+.01) +(benthic_layer_3(3,j)*real(j)) !For relative depth below the water column 
    benthic_layer_3(8,j)=benthic_layer_3(2,j)-((benthic_layer_3(2,j)/dx_1)*pdx)
    !benthic_layer_3(8,j)=state_var_time_3(2)-((state_var_time_3(2)/dx_1)*pdx)
    benthic_layer_3(9,j)=(benthic_layer_3(2,j)/(svol*10**6)) - (pdx*(benthic_layer_3(2,j)/(svol*10**6))/dx_1) 
    !benthic_layer_3(10,j)=benthic_layer_3(8,j)/tot_rhos
    !benthic_layer_3(11,j)=benthic_layer_3(10,j)/a
    allocate(bottom_out(2,rtm))
    bottom_out(1,j) = benthic_layer_3(8,j)/tot_rhos
    bottom_out(2,j) = bottom_out(1,j)/a
    ...
    !mass_out(1:4,j)= output_mass(:,:)
     ...
     !used for the depostion into the benthic layer from second layers
        if(timeout_1(7,j)>timeout_1(4,j)) then   
                ...
                !state_att_2(6,1)=0
               ! benthic_layer_3(10,j)= benthic_layer_3(8,j)/(state_att_1(6,3)+state_att_1(6,2))
        if(benthic_layer_3(7,j)>= maxlyrd_b) then
            benthic_layer_3(7,j)= abs(maxlyrd_b-(state_var_time_3(3)*real(j))) !For relative depth below the water column
        
        !benthic_layer_3(8,j)= benthic_layer_3(4,j)-(benthic_layer_3(8,j)/state_var_time_3(4))
        end if
...
!    conc_time_3(j) = benthic_layer_3(8,j)
    ...
    !end if 
    !print*, sed_1(:,j) 
    write(15,*) j, timeout(8,j), timeout_1(8,j), benthic_layer_3(8,j), bottom_out(1,j), bottom_out(2,j)
    write(25,*) j, timeout(9,j), timeout_1(9,j), benthic_layer_3(9,j)
    !print*,timeout(8,j)
    !print*,timeout_1(8,j)
