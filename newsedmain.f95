...
open(unit=15, file="C:\Users\kvenable\Output\mass01MAY20b.txt",status="new")
open(unit=25, file="C:\Users\kvenable\Output\conc01MAY20b.txt", status="new")
...
!num_par_1 = conc_i*lc*a
    ...
    allocate(benthic_layer_3(11,rtm))
    ...
    !mass_out(1:4,j)= output_mass(:,:)
     ...
     !used for the depostion into the benthic layer from second layers
        ...
        if(benthic_layer_3(7,j)>= maxlyrd_b) then
            benthic_layer_3(7,j)= maxlyrd_b-(state_var_time_3(5)*real(j)) !For relative depth below the water column
...
!    conc_time_3(j) = benthic_layer_3(8,j)
    ...
    !end if 
    !print*, sed_1(:,j) 
    write(15,*) j, timeout(8,j), timeout_1(8,j), benthic_layer_3(8,j), benthic_layer_3(10,j),benthic_layer_3(11,j)
