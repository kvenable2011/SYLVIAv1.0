...
common /shear/tb 
open(unit=10, file="C:\Users\kvenable\Output\GlobalStateFile22JAN21d.txt",status="new")
call cpu_time(start)
!call date_and_time
open(unit=15, file="C:\Users\kvenable\Output\XY22JAN21d.txt",status="new")
open(unit=25, file="C:\Users\kvenable\Output\mass22JAN21d.txt", status="new")
open(unit=35, file="C:\Users\kvenable\Output\conc22JAN21d.txt", status="new")
open(unit=45, file="C:\Users\kvenable\Output\mad22JAN21d.txt", status="new")
open(unit=55, file="C:\Users\kvenable\Output\ml22JAN21d.txt", status="new")
open(unit=65, file="C:\Users\kvenable\Output\inputmatrixpass22JAN21d.txt", status="new")
...
DO k=1,nr
 read(1,*)comid,l,da,h,q,u,akm
 mydata(1:7,k)=(/real(comid),l,da,h,q,u,akm/)
...
! write(10,*) tica
 ...
 !ks(k) = max(ks1,ks2)
 write(*,*) ks1, ks2, ks
 tic = mydata(2,k)/mydata(6,k) !------> time in channel based on l/u km/fps to sec 
...
DO  k=1,nr
 ...
 dx = 0
 dx_1 = 0 
 dx = mydata(6,k)/3.2804 *1!displacement in 1 sec (m/s)
 dx_1= dx*1*86400 !displacement in 1 day 
 write(10,*) "Total displacement in 1 sec is=" ,dx, runtime(6,k)
 ...
 if (k == 1) then 
    ...
    svol= (mydata(5,1)/35.3147)/dlayer  !IN CUBIC METERS
    print*, "svol=", svol
...
! start_j = 1
    ...
    start_j = J1
        ...
        do j= start_j,int(sumcount)!int(final_j)!,tinc
            if (j == rtm) then
            exit
!            !-----> For the j time step iteration close !Need to place conditional statement
!            !if (rtm >= sumcount) then 
...
!            exit !Use these parameters to calc next pdx final displacement in network
        ...
        !end if 
        if (j>sumcount) then 
        new_end_step =sumcount+int(next_time)
