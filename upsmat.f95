...
real :: state_att_1(4,3), mad(3,1)!, m_a(:,:)
...
real, allocatable :: m_a(:,:)
real, parameter:: rhow=.998, g=9.807, vis=.001
integer, parameter::ns=3,k=12
real :: rhos, q, d_m_1, d_m_2, d_m_3, start, finish, m1, m2, m3
...
open(unit=10, file="C:\Users\kvenable\Output\GlobalStateFile20MAY20e.txt",status="new")
call cpu_time(start)
!call date_and_time(
open(unit=15, file="C:\Users\kvenable\Output\XY20MAY20e.txt",status="new")
open(unit=25, file="C:\Users\kvenable\Output\mass20MAY20e.txt", status="new")
open(unit=35, file="C:\Users\kvenable\Output\conc20MAY20e.txt", status="new")
...
do j=1,rtm,tinc
    ...
    allocate(m_a(3,rtm), timeout(3,rtm), timeout_1(3,rtm), benthic_layer_3(3,rtm))
    timeout(1,1)=(state_var_time_1(4)-dlayer)+(tsv_1*real(j-1))                !y displacement
    timeout(2,1)=state_var_time_1(2)-(d_m_1*pdx)!change in mass flux
    timeout(3,1)=timeout(2,j)/svol!concentration scaling factor
    !allocate(timeout_1(3,1))
    timeout_1(1,1)=(state_var_time_2(4)-dlayer)+(tsv_2*real(j-1))                !y displacement
    ...
    !allocate(benthic_layer_3(5,1))
    benthic_layer_3(1,1)=((h+.01)+(tsv_3*real(j-1))) !y displacement
    ...
    benthic_layer_3(3,1)=benthic_layer_3(2,j)/svol!concentration scaling factor
    benthic_layer_3(4,1)= benthic_layer_3(2,j)/tot_rhos!accummulated total vol in benthic layer
    benthic_layer_3(5,1)= 0
    call ma(mad)
    m_a(1,j)=mad(1,1)
    m_a(2,j)=mad(2,1)
    m_a(3,j)=mad(3,1)
 if (j>=2) then
    ...
    timeout(2,j)=m_a(1,j)-(d_m_1*pdx)!change in mass flux
    ...
    timeout_1(2,j)=m_a(2,j)-(d_m_2*pdx)!change in mass flux
    ...
    !((maxlyrd_b-0.1)+.01) +(benthic_layer_3(3,j)*real(j)) !For relative depth below the water column 
    benthic_layer_3(2,j)=m_a(2,j)-(d_m_3*pdx)!change in mass flux
